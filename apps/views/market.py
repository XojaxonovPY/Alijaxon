...
from apps.forms import ThreadModelForm
from apps.models import Category, Product, User, Region, Order, AdminSetting, Thread
...
def diagram_statistic_view(request):
    ...
    data = {
        "regions": list(regions.values_list('name', flat=True)),
        "numbers": list(regions.values_list('order_count', flat=True))
    }
